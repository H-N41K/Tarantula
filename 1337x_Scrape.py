...
for i in range(1,151):
    print(f'##############  PAGE {i} #################')
    url = f"http://1337x.to/cat/Movies/{i}/"
    try:
        driver.get(url)
    ...
    for row in page_soup.find_all('tr')[1:]:
        ...
        if(len(row)) == 0:
            ...
            con.execute(f"INSERT INTO torrents (torrentname, torrenturl, seederscount, leecherscount, torrentsize) VALUES ('{torrent_name}', '{torrent_url}', {seeders_count}, {leechers_count}, '{torrent_size}')")
            con.commit()
        else:
            print('DUPLICATE:: {} | {} | {} | {} | {}\n'.format(torrent_name, torrent_url, seeders_count, leechers_count, torrent_size))	
    time.sleep(3)
driver.close()
